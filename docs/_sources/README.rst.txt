...
For that, you need both accelerometers. Now, with `gravity riding everything`_, you have two sets of three axis positioning data that can be compared to establish positional planes in three dimensional space that must intersect eachother or be parallel. Seems hard, but it's just what we need. And yes, unfortunately auto rotate will stop working if you find yourself falling through a vacuum at terminal velocity, but that is a known design flaw.

Yes, that is a lot of math. Yes I did start that way. After writing it, I noticed that you don't actually need to do that mathy bit. The hinge creates one fixed relationship around which the others pivot and the whole mess can be boiled down to ratio comparison. Yay, down to two dimensions. So I ripped out the mathy bits to caculate angular data and just sort of wing it with the ratios. Efficiency is a virtue.

That is just what `src/modewatcher.py`_ will do for you minus all of the dbus complexity. I originally wrote it for 19.10 xubuntu. The only thing I needed to change was change disp from "eDP1" to "eDP-1". 
